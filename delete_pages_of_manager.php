...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM manager_access_pages WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href = 'delete_pages_of_manager.php';</script>";
    } else {
...
function resequenceTable($conn) {
    ...
    $update_ids_sql = "SET @new_id = 0; 
                       UPDATE manager_access_pages SET id = (@new_id := @new_id + 1) ORDER BY id ASC";
    if ($conn->multi_query($update_ids_sql)) {
        ...
        $conn->query("ALTER TABLE manager_access_pages AUTO_INCREMENT = 1");
    } else {
...
$sql = "SELECT * FROM manager_access_pages ORDER BY id ASC";
    ...
    <script>
        function confirmDeletion(delete_id) {
            if (confirm("Are you sure you want to delete this record?")) {
                window.location.href = 'delete_pages_of_manager.php?delete_id=' + delete_id;
            }
        }
    </script>
</head>
